...
from Mask_RCNN.mrcnn import email
...
def detect(file1):
    im = cv.imread('satellite-img-4.png', cv.IMREAD_COLOR)
    ...
    im = cv.imread(file1, cv.IMREAD_COLOR)
    width, height, channels = im.shape
    TP = width * height
    lower_blue = np.array([25,50,35])
    upper_blue = np.array([60,100,70])
    ...
    if (difference > 0.1800):
        ...
        print("Sending Email to the concerned authorities")
        email.mail()
    elif (difference < 0):
